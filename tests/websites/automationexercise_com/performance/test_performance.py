...
from tests.websites.automationexercise_com.helpers import AutomationSignupPage
...
async def test_homepage_navigation_performance_metrics(
    ...
    assert metrics["time_to_first_byte_ms"] < max_ttfb_ms, (
        f"Median TTFB {metrics['time_to_first_byte_ms']:.1f} ms exceeds "
        f"{max_ttfb_ms:.1f} ms across {sample_count} sample(s)"
    )
    assert metrics["load_event_complete_ms"] < max_load_ms, (
        f"Median full load {metrics['load_event_complete_ms']:.1f} ms exceeds "
        f"{max_load_ms:.1f} ms across {sample_count} sample(s)"
    )
